...
switch ($request) {
    ...
    case 'get-contacts':
        if ($method == 'POST') {
            ...
            echo json_encode(array("status" => "success", "data" => $contacts));
            exit();
        }
        echo json_encode(array("status" => "error", "data" => "Invalid request."));
        exit();
    case 'get-contacts-by-department':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/ContactsService.php';
            if (isset($_POST['department_id'])) {
                $contacts = ContactsService::getContactFromDepartment($_POST['department_id']);
                echo json_encode(array("status" => "success", "data" => $contacts));
                exit();
            }
        }
    ...
    case 'create-contact':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/ContactsService.php';
            if (isset($_POST['department_id']) && isset($_POST["extension"]) && isset($_POST['country_number']) && isset($_POST['contact']) && isset($_POST['contact_email']) && isset($_POST['department_only'])) {
                $create = ContactsService::createContact($_POST['country_number'], $_POST['contact'], $_POST['contact_email'], $_POST['department_id'], $_POST["extension"], $_POST["department_only"]);
                if ($create) {
    ...
    case 'update-contact':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/ContactsService.php';
            if (isset($_POST['department_id']) && isset($_POST['country_number']) && isset($_POST['contact']) && isset($_POST['contact_email']) && isset($_POST["extension"]) && isset($_POST['contact_id']) && isset($_POST['department_only'])) {
                $update = ContactsService::updateContact($_POST['contact_id'], $_POST['country_number'], $_POST['contact'], $_POST['contact_email'], $_POST["extension"], $_POST['department_id'], $_POST['department_only']);
                if ($update) {
    ...
    case 'create-department':
        if ($method == 'POST') {
            ...
            if (isset($_POST['department_name']) && isset($_POST["department_code"]) && isset($_POST['department_extensions'])) {
                $create = DepartmentService::createDepartment($_POST['department_name'], $_POST["department_code"], $_POST['department_extensions']);
                if ($create) {
    ...
    case 'update-department':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/DepartmentService.php';
            if (isset($_POST['department_id']) && isset($_POST['department_name']) && $_POST["department_code"] && isset($_POST['department_extensions'])) {
                $update = DepartmentService::updateDepartment($_POST['department_id'], $_POST['department_name'], $_POST["department_code"], $_POST['department_extensions']);
                if ($update) {
    ...
    case 'contacts':
        ...
        exit();
    case 'report':
        require __DIR__ . '/public/views/report.php';
        break;
    case 'send-report':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/ReportService.php';
            if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['message'])) {
                $report = ReportService::sendReport($_POST['name'], $_POST['email'], $_POST['message']);
                if ($report) {
                    header('Location: /report?status=success');
                    exit();
                } else {
                    header('Location: /report?status=error');
                    exit();
                }
            }
        }
        header('Location: /report?status=error');
        exit();
    case 'get-gest-contacts':
        if ($method == 'POST') {
            include_once __DIR__ . '/private/services/ContactsService.php';
            $contacts = ContactsService::getContactsForGests($_POST['department']);
            echo json_encode(array("status" => "success", "data" => $contacts));
            exit();
        }
        echo json_encode(array("status" => "error", "data" => "Invalid request."));
